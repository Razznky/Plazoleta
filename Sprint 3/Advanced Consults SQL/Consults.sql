-- Vista para centralizar la ubicación geográfica de los restaurantes
...
-- 1. Listar el nombre de todos los platos junto con su categoría.
SELECT 
...
-- 2. Mostrar la ubicación de los restaurantes.
...
-- 4. Cantidad de platos registrados por cada categoría.
SELECT 
    cp.nombre AS Categoria, 
...
-- 5. [HU5] Pedidos detallados agrupados por restaurante.
SELECT 
    r.nombre_comercial AS Restaurante, 
...
-- 6. [HU6] Total de ventas generadas por mes.
SELECT 
...
-- 7. [HU7] Estadísticas de platos vendidos por restaurante.
SELECT 
    r.nombre_comercial AS Restaurante, 
    p.nombre AS Plato, 
...
-- 8. Turnos de empleados que inician entre las 08:00 AM y 10:00 AM.
SELECT 
...
-- 9. Pedidos con costo de envío > $5,000 y ya entregados.
SELECT 
    pe.pedido_id, 
    CONCAT(c.nombres, ' ', c.apellidos) AS Cliente, 
    pe.costo_envio, 
...
-- 10. Reporte de platos 'Agotado' con su categoría.
SELECT 
    p.nombre AS Plato, 
    p.codigo_interno, 
    cp.nombre AS Categoria
